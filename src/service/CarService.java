...
public class CarService {
  ...
  /**
   * 車の新規登録時に割り振られるIDを作るメソッド
   * 
   * @param cars 車一覧
   * @return 車一覧のIDで一番大きい値+1を返す
   */
  public int generateNextCarId(List<Car> cars) {
    int maxId = 0;
    for (Car car : cars) {
      if (car.getId() > maxId) {
        maxId = car.getId();
   ...
   */
  public boolean updateCarMileage(List<Car> cars, int carId, int mileage) {
   ...
   * @param cars 車の一覧
   * @param carId 更新したい車のID
   * @param status 更新後の車の状態
   * @return carIdが存在する車のIDで更新に成功した場合はtrue, 存在せず、更新に失敗する場合はfalse
   */
  public boolean updateCarStatus(List<Car> cars, int carId, CarStatus status) {
    Car car = findCarById(cars, carId);
    if (car != null) {
  ...
  public List<Car> loadFromCsv() throws IOException {
    ...
    try (BufferedReader reader = new BufferedReader(new FileReader(CARS_CSV))) {
      ...
      while ((line = reader.readLine()) != null) {
        ...
        // carsに読み込んだCarを追加
        cars.add(new Car(id, vehicleModel, color, maker, status, cumulativeMileage, currentUserId,
            transmission));
      }
    } catch (Exception e) {
  ...
  public void saveToCsv(List<Car> cars) throws IOException {
    ...
    try (BufferedWriter writer = new BufferedWriter(new FileWriter(CARS_CSV))) {
      writer
          .write("id,vehicleModel,color,maker,status,cumulativeMileage,currentUserId,transmission");
      // 改行
      writer.newLine();

      // forでCarごとにフィールドを取り出し、文字列に変換、csvに書き込む
      // id | vehicleModel | color | maker | status | cumulativeMileage | currentUserId |
      // transmission
      for (Car car : cars) {
        ...
        String currentUserId =
            (car.getCurrentUserId() == null) ? "" : String.valueOf(car.getCurrentUserId());
        String transmission = car.getTransmission().name();

        writer.write(String.join(",", id, vehicleModel, color, maker, status, cumulativeMileage,
            currentUserId, transmission));
        writer.newLine();
      }
    } catch (Exception e) {
  ...
  public boolean canReturnCar(Car car, int userId) {
    if (car.getCurrentUserId() != null && car.getStatus() == CarStatus.RENTED
        && car.getCurrentUserId() == userId) {
      return true;
    }
    return false;
  }
