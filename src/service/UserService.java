...
public class UserService {
  ...
  /**
   * ユーザー一覧から名前でユーザーの情報を取り出すメソッド
   * 
   * @param users ユーザーデータ一覧
   * @param name 取得したいユーザーの名前
   * @return 存在するユーザーの名前だったらそのユーザーを返す。存在しない場合はnullを返す。
   */
  ...
  /**
   * ユーザー登録時に割り振られるIDを作るメソッド
   * 
   * @param users ユーザー一覧
   * @return ユーザー一覧のIDで一番大きい値+1を返す
   */
  public int generateNextUserId(List<User> users) {
    int maxId = 0;
    for (User user : users) {
      if (user.getId() > maxId) {
        maxId = user.getId();
